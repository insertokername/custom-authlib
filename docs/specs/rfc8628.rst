...
.. meta::
    :description: Python API references on RFC8628 DeviceAuthorizationEndpoint
        and DeviceCodeGrant with insertokname-authlib implementation.

.. module:: insertokname-authlib.oauth2.rfc8628
...
To integrate with insertokname-authlib :ref:`flask_oauth2_server` or :ref:`django_oauth2_server`,
...
There are two missing methods that developers MUST implement::
    ...
    from insertokname-authlib.oauth2.rfc8628 import DeviceAuthorizationEndpoint
...
With insertokname-authlib ``.register_grant``, we can add ``DeviceCodeGrant`` easily.
But first, we need to implement the missing methods::

    from insertokname-authlib.oauth2.rfc8628 import DeviceCodeGrant
